...
neighbors d i = concat $ zipWith uncurry [up, down, left, right] (repeat (d,i))
  where
    up    d (r,c) = if r > 0   then [(r-1,c)] else []
    down  d (r,c) = if r < d-1 then [(r+1,c)] else []
    left  d (r,c) = if c > 0   then [(r,c-1)] else []
    right d (r,c) = if c < d-1 then [(r,c+1)] else []
...
chain d pn pl i = chain' xs d pn pl i
  where xs = maybe [] (\pl' -> if isSamePlayer pl pl' then [i] else []) (colorOf pn i)
...
sequenceMoves pn pl is = foldr nextPosition pn $ zip (cycle [pl, oppositePlayer pl]) is
  where nextPosition (pl, i) pn = addPlay pl (Move i) pn
...
show_it g = putStrLn $ displayGame g
